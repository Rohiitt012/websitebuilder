...
The project uses **Prisma** with **PostgreSQL**. To connect your pgAdmin/PostgreSQL database:

1. Copy `.env.example` to `.env` and set your connection string:
   ```bash
   cp .env.example .env
   ```
2. Edit `.env` and set `DATABASE_URL` to your PostgreSQL details:
   ```
   DATABASE_URL="postgresql://USER:PASSWORD@HOST:5432/DATABASE_NAME?schema=public"
   ```
   Example: `postgresql://postgres:mypassword@localhost:5432/admin_dashboard?schema=public`

3. Create the database in pgAdmin (or `createdb admin_dashboard`), then run migrations:
   ```bash
   npx prisma migrate dev --name init
   ```
4. Use the database in your app by importing the client:
   ```ts
